...
function fetchCards(category = 'all') {
    ...
    filteredCards.forEach(card => {
        ...
        const stockAvailable = parseInt(card.stock);
        let stockStatus = '';

        if (stockAvailable === 0) {
            stockStatus = `<p class="stock-status out-of-stock">Out of Stock</p>`;
        } else if (stockAvailable < 10) {
            stockStatus = `<p class="stock-status low-stock">${stockAvailable} stock left</p>`; // Apply low-stock class
        }
            ...
            <div class="col-md-4 mb-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <button class="btn btn-primary" onclick="addToCart('${card.title}', ${card.price})" ${stockAvailable === 0 ? 'disabled' : ''}>Add to Cart</button>
